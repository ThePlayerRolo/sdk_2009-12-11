...
#define HBM_MEM_SIZE_SOUND_MINUS_STACK	\
	(HBM_MEM_SIZE_SOUND - sizeof ::mThreadStack)
...
{
	// .rodata
	char const WT_FILENAME[] = "wt\\HomeButtonSe.wt";
	char const PCM_FILENAME[] = "wt\\HomeButtonSe.pcm";

	// .bss
	byte8_t mThreadStack[0x800] ATTR_UNUSED; /* New! */
...
void AudioFrameCallback()
{
	...
	BOOL result ATTR_UNUSED = OSSendMessageAny(
...
void *AudioSoundThreadProc(void *)
{
	...
	while (true)
	{
		...
		if (OSReadMessageAs(message, u32) == AudioThreadMsg_RunAudioFrame)
		{
			HBMSEQRunAudioFrame();
			HBMSYNRunAudioFrame();
		...
		else if (OSReadMessageAs(message, u32) == AudioThreadMsg_Exit)
		{
			break;
		}
	}
...
void UpdateSeqPlayerList(SeqPlayerList *list)
{
	...
	for (SeqPlayer *next; current; current = next)
	{
		...
		if (current->activeFlag
		    && HBMSEQGetState(&current->seq) == HBM_SEQ_STATE_0
...
void InitAxSound(void const *soundData, void *mem, u32 memSize)
{
	...
	work->pcm = nullptr;
	work->mpThreadStack =
		POINTER_ADD_TYPE(byte8_t *, mem, HBM_MEM_SIZE_SOUND_MINUS_STACK);
	...
	result = OSCreateThread(&work->mThread, AudioSoundThreadProc, nullptr,
	                        reinterpret_cast<byte_t *>(work->mpThreadStack)
	                            + memSize - HBM_MEM_SIZE_SOUND_MINUS_STACK,
	                        memSize - HBM_MEM_SIZE_SOUND_MINUS_STACK, 4,
	                        OS_THREAD_NO_FLAGS);
	if (!result)
		return;
...
void ShutdownAxSound()
{
	...
	BOOL result = OSJamMessageAny(&sWork->mMsgQueue, AudioThreadMsg_Exit,
	                              OS_MESSAGE_FLAG_PERSISTENT);
